...
Write a Python program to find the smallest substring that contains all the unique characters present in the given string.
...
# String ke unique characters find karo
for i in n:
    if i not in u:
        u += i

print(u)  # unique characters
...
for i in range(len(n)):
    ...
    for j in range(i + 1, len(n) + 1):
        ...
        # Check karo ki substring mein saare unique characters hain
        for ch in u:
        ...
        # Agar saare characters present hain
        if flag:

            # Pehla valid substring ya smaller substring
